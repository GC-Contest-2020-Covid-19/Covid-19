...
module.exports = {
    ...
    scrapeMyths: async function () {
        ...
        const myths = await page.evaluate(() => {
            return Array.from(
                document.querySelectorAll("#PageContent_C003_Col01 div.sf-content-block.content-block div")
            )
                .map((div) => ({
    ...
    scrapeEDX: async function (query) {
        const browser = await puppeteer.launch({
            args: ["--no-sandbox"],
        });
        ...
        //setting timeout to 0 ensures that there is no network timeout in case of slow network connections
        await page.goto(`https://www.edx.org/course?search_query=${query}`, {
            waitUntil: "load",
            timeout: 0,
        });

        // get data
        const selector = [".label", ".title-heading"];
        let data = [];
        for (let i = 0; i < selector.length; i++) {
            data.push(
                extractContent(
        ...
        data.push(
            await page.evaluate(function () {
                const listItems = Array.from(document.querySelectorAll(".course-link"));
                return listItems.map((li) => li.getAttribute("href"));
            })
        );

        data[0] = filterEdx(data[0]);
...
function filterEdx(array) {
    let rv = [];
    for (let i = 0; i < array.length; i++) {
        rv.push(array[i].replace("Schools and Partners:\n", ""));
    }
    return rv;
}
