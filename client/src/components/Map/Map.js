...
export const CoronaMap = () => {
    ...
    const requested = useSelector((state) => state.map.requested);

    // something was requested
    if (requested) {
        // last request was successful
        if (
            (testStations.testStations.length > 0 || foodBanks.foodBanks.length > 0) &&
            testStations.testStations.length === testStations.results &&
            foodBanks.foodBanks.length === foodBanks.results
        ) {
            // process icons
            ...
            if (
                Math.abs(user.coordinates[0] - testStations.testStations[0].lat) > 10 ||
                Math.abs(user.coordinates[1] - testStations.testStations[1].lng) > 10
            ) {
            ...
            return (
                <div className={"custom-ml-5"}>
                    <h3>
                        Found {testStations.results} test stations and {foodBanks.results} food banks.
                    </h3>
                    <p>Blue = Test Station, Red = Foodbanks</p>
                    <br />
            ...
            !(testStations.status || foodBanks.status) ||
            (testStations.results === 0 && foodBanks.results === 0)
        ) {
            return (
                <div className={"custom-ml-5"}>
                    <h3>Nothing found.</h3>
