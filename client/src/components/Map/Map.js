...
export const CoronaMap = () => {
    ...
    if(requested){
        ...
        if ((testStations.testStations.length > 0 || foodBanks.foodBanks.length > 0) && (testStations.testStations.length === testStations.results && foodBanks.foodBanks.length === foodBanks.results)){
        ...
        }else if (!(testStations.status || foodBanks.status) || (testStations.results === 0 && foodBanks.results === 0) ) {
            return (
                <div className={'custom-ml-5'}>
                    <h3>Nothing found.</h3>
                </div>
