...
const Myths = ({ setupMyths, myths }) => {
    ...
    return myths.length > 0 ? (
        <section id="myths" className="custom-px-3">
            <h2 className="has-text-centered is-size-2">Common myths about the virus and disease</h2>
            {myths?.map((myth, index) => {
                return (
                    <div key={index} className="box">
                        ...
                        <p className="is-size-4-desktop is-size-5-tablet is-size-6-mobile">{myth.paragraph}</p>
                        <br />
                    </div>
                );
            })}
